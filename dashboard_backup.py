...
def main():
    ...
    # Main navigation - Reorganizado para mejor UX
...
Métricas: Accuracy, Precision, Recall, F1-Score
    ...
    with tab8:
        st.header("⚙️ Configuración Manual de Quiniela")
        st.markdown("Selecciona manualmente los partidos que formarán parte de la Quiniela de esta jornada")
        
        # Obtener todos los partidos disponibles de la temporada
        with st.spinner("Cargando partidos disponibles..."):
            # Hacer dos llamadas separadas para cada liga ya que la API no acepta múltiples league_ids
            la_liga_data = make_api_request(f"/matches/", {"season": current_season, "league_id": 140, "limit": 25})
            segunda_data = make_api_request(f"/matches/", {"season": current_season, "league_id": 141, "limit": 25})
            
            # Combinar resultados - La API devuelve directamente una lista, no un objeto
            all_matches_data = None
            matches = []
            if la_liga_data and isinstance(la_liga_data, list):
                matches.extend(la_liga_data)
            if segunda_data and isinstance(segunda_data, list):
                matches.extend(segunda_data)
            
            if matches:
                all_matches_data = {"matches": matches}
                    ...
                    for i, match in enumerate(la_liga_matches[:20]):  # Limitar a 20 para no saturar
                        ...
                        if match_date:
                            ...
                            try:
                                ...
                                formatted_date = match_date[:16]
                        else:
                            formatted_date = "Sin fecha"
                        
                        match_label = f"{match.get('home_team', 'Equipo A')} vs {match.get('away_team', 'Equipo B')} - {formatted_date}"
                        
                        if st.checkbox(match_label, key=f"la_liga_{i}"):
                            selected_la_liga.append({
                                'match_id': match.get('id'),
                                'home_team': match.get('home_team', 'Equipo A'),
                                'away_team': match.get('away_team', 'Equipo B'),
                                'match_date': match.get('match_date', ''),
                                'league': 'La Liga'
                            })
                else:
